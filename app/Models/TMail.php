...
class TMail extends Model {
    ...
    private static function storeEmail($email, $password, $recoveryMail) {

        $password = ($password) ? $password : Str::random(6, 'lowercase');
        $recoveryMail = ($recoveryMail) ? $recoveryMail : '';
    ...
    public static function createCustomEmail($username, $domain, $password = false, $recoveryMail = false) {
        ...
        if (in_array($domain, $domains)) {
            ...
            TMail::storeEmail($email, $password, $recoveryMail);
            return $email;
        } else {
    ...
    public static function generateRandomEmail($store = true) {
        ...
        if ($store) {
            TMail::storeEmail($email, false, false);
        }
        return $email;
    }
    ...
    public static function storage()
    {
            ...
            if($email){
                ...
                $storageUsed = $bodySize + $attachmentStorage;
                return number_format($storageUsed,2);
