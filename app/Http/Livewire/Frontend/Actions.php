...
class Actions extends Component {
    ...
    public $in_app = false;
    public $user, $domain, $domains, $email, $emails, $captcha, $recoveryMail, $password;

    protected $listeners = ['syncEmail', 'checkReCaptcha3', 'componentBTriggeredFromNav'];
    ...
    public function create() {
        ...
        $this->email = TMail::createCustomEmail($this->user, $this->domain, $this->password, $this->recoveryMail);
    ...
    private function checkEmailExist() {
        $check = Log::where('email', $this->user . '@' . $this->domain)->count();
        if ($check > 0) {
            return false;
        }
        return true;
    ...
    public function componentBTriggeredFromNav()
    {
        $this->in_app = true;
