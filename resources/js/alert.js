document.addEventListener('showAlert', (type) => {
  ...
  el.innerHTML = `
    <div x-data="{ open: false }" x-init="() => { setTimeout(() => { open = true }, 100); setTimeout(() => { open = false }, 3000) }" class="fixed flex items-center justify-center bottom-0 right-0 w-full z-50">
      <div class="w-full max-w-sm m-2 py-3 px-4 overflow-hidden shadow-lg rounded-md flex items-center border ${shades[type.detail.type]
    }"
        x-show="open"
      ...
      ${icons[type.detail.type]}
        <div class="ml-4 flex-1">
            <div class="text-lg text-gray-600 font-semibold">${document.querySelector(`.language-helper .${type.detail.type}`)
      .innerText
    }</div>
            <div class="text-sm text-gray-500">${type.detail.message}</div>
        </div>
