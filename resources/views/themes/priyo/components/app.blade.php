                ...
                <div x-show="!show" class="list">
                    ...
                    <div class="messagesListArea">
                        <div class="messages flex flex-col justify-end">
                            @foreach ($messages2 as $i => $message)
                                ...
                                <div x-on:click="show = true; id = {{ $message['id'] }}; document.querySelector('button.delete').setAttribute('wire:click', 'delete({{ $message['id'] }})')"
                                    class="flex items-center gap-3 hover:bg-gray-200 border-b border-dashed py-4 px-7 cursor-pointer {{ $message['isRead'] ? 'mailRead':'' }}"
                                    data-id="{{ $message['id'] }}" wire:click="updateView({{ $message['id'] }})">
                                ...
                                <div class="w-full flex justify-between items-center">
                                    ...
                                    <div class="flex gap-3">
                                        ...
                                        <button
                                            ...
                                            class="delete border-b" wire:click="delete({{ $message['id'] }})">{{ __('Delete') }}</button>
                                    </div>
                                </div>
                            </div>
            ...
            @if ($initial)
                ...
                <div x-show="!show" class="list">
                    ...
                    <div class="messages flex flex-col justify-end">
                        ...
                        <div x-on:click="show = true; id = 99999999" class="flex items-center gap-3 hover:bg-gray-200 border-b border-dashed py-4 px-7 cursor-pointer">
                            <div class="w-1/2 md:w-3/12">
                                {{ config('app.settings.mailName') }}
                                <div class="text-xs overflow-ellipsis">{{ config('app.settings.mailFrom') }}</div>
                            </div>
                            <div class="w-1/2 md:w-7/12">{{ config('app.settings.mailSubject') }}</div>
                            <div class="hidden md:block w-full md:w-2/12">
                ...
                <div x-show="show" class="message">
                    <div x-show="id === 99999999" id="message-99999999" class="message min-h-tm-half">
                        ...
                        <div class="border-t border-b border-dashed py-4 px-7">
                            {{ config('app.settings.mailFrom') }}
                        </div>
                        <div class="text-wrap py-4 px-7">
                            <div class="w-full flex flex-grow min-h-tm-half">
                                {!! nl2br(config('app.settings.mailDetails')) !!}
                            </div>
