            ...
            <div class="m-auto">
                ...
                <div class="bg-white rounded-lg p-10">
                    @if (Session::has('error'))
                        <div
                            class="bg-red-50 w-full flex justify-between items-center rounded-lg px-4 py-3 text-sm mb-5">
                    ...
                    <form action="{{ route('unlock') }}" class="flex justify-center items-center gap-2" method="post">
                        ...
                        <input type="password" name="password" id="password"
                            class="flex-1 w-full rounded-md px-4 py-2 text-sm outline-none border-1 border-gray-200 focus:border-gray-200 focus:shadow-none"
                            placeholder="{{ __('Password') }}">
                        <button type="submit" class="rounded-md px-4 py-2 text-sm border-1 text-white"
                            style="border-color: {{ config('app.settings.colors.secondary') }}; background-color: {{ config('app.settings.colors.secondary') }}">{{ __('Unlock') }}</button>
                    </form>
                </div>
            </div>
        </div>
