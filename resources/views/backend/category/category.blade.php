<x-jet-form-section submit="update">
    ...
    <x-slot name="description">
        {{ __('You can manage the Custom Category of your TMail here.') }}
    </x-slot>
        ...
        @if($addCategory || $updateCategory)
            <div class="col-span-6 flex justify-between">
                ...
                <div class="bg-gray-200 rounded-md px-4 py-2 flex items-center gap-3">
                    ...
                    <span>{{ __('Add translations for') }} {{ $category['lang_text'] }} ({{ $category['lang'] }})</span>
                </div>
                @endif
            </div>
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="title" value="{{ __('Name') }}" />
                <x-jet-input id="title" type="text" class="mt-1 block w-full" placeholder="Category Name" wire:model.defer="category.title"/>
                <x-jet-input-error for="category.title" class="mt-2" />
            </div>
            @if(!isset($category['lang']))
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="slug" value="{{ __('Slug') }}" />
                <x-jet-input id="slug" type="text" class="mt-1 block w-full" placeholder="Category Slug" wire:model.defer="category.slug"/>
                <x-jet-input-error for="category.slug" class="mt-2" />
            </div>
            @endif
            <div class="col-span-6">
                <x-jet-label for="content" value="{{ __('Content') }}" />
                <div class="mt-1"><div id="quill-content">{!! (!$addCategory) ? $category['content'] : '' !!}</div></div>
                <textarea id="content" class="hidden" wire:model.defer="category.content"></textarea>
                <x-jet-input-error for="category.content" class="mt-2" />
            </div>
            <div class="col-span-6">
                <x-jet-label value="{{ __('Meta Tags') }} ({{ __('Optional') }})" />
                @foreach($category['meta'] as $key => $meta)
                <div class="flex mt-2">
                    <div>
                        <label class="block font-medium text-gray-700 text-xs">{{ __('Name') }}</label>
                        <div class="relative">
                            <select class="form-input rounded-md shadow-sm mt-1 block w-full cursor-pointer" wire:model.defer="category.meta.{{ $key }}.name">
                        ...
                        <x-jet-input-error for="category.meta.{{ $key }}.name" class="mt-2" />
                    </div>
                    <div class="flex-1 ml-3">
                        <label class="block font-medium text-gray-700 text-xs">{{ __('Content') }}</label>
                        <div class="flex">
                            <x-jet-input type="text" class="mt-1 block w-full" wire:model.defer="category.meta.{{ $key }}.content"/>
                        ...
                        <x-jet-input-error for="category.meta.{{ $key }}.content" class="mt-2" />
                    </div>
                </div>
                @endforeach
            ...
            <div class="col-span-6">
                ...
                <x-jet-input-error for="category.header" class="mt-2" />
            </div>
            @if(isset($category['id']) && !isset($category['lang']))
            <div class="col-span-6">
            ...
            @foreach($categories as $category)
                <div class="bg-gray-800 text-white rounded-md mt-3 flex flex-col md:flex-row justify-between items-center">
                    <div class="flex px-5 py-4 overflow-y-hidden">
                        ...
                        <div class="px-3">-</div>
                        <div><small classs="text-xs"><a href="{{ env('APP_URL') }}{{ ($category->parent_id) ? '/' . $category->parent_slug : '' }}/{{ $category->slug }}" target="_blank">{{ env('APP_URL') }}{{ ($category->parent_id) ? '/' . $category->parent_slug : '' }}/{{ $category->slug }}</a></small></div>
                    </div>
                    <div class="hidden md:flex">
                        <div class="cursor-pointer px-5 py-4 bg-blue-900" wire:click="showUpdate({{ $category->id }})"><i class="fas fa-edit"></i></div>
                        <div class="cursor-pointer px-5 py-4 bg-red-900 rounded-r-md" wire:click="delete({{ $category->id }})"><i class="fas fa-trash-alt"></i></div>
                    </div>
                    <div class="md:hidden w-full grid grid-cols-2 text-center">
                        <div class="cursor-pointer px-5 py-2 bg-blue-900 rounded-l-md" wire:click="showUpdate({{ $category->id }})"><i class="fas fa-edit mr-2"></i>{{ __('Edit') }}</div>
                        <div class="cursor-pointer px-5 py-2 bg-red-900 rounded-r-md" wire:click="delete({{ $category->id }})"><i class="fas fa-trash-alt mr-2"></i>{{ __('Delete') }}</div>
                    </div>
                </div>
            @endforeach
            </div>
    ...
    <x-slot name="actions">
        ...
        @if($addCategory || $updateCategory)
            ...
            @else
                @if(isset($category['lang']))
                    @if($this->isTranslated($category['lang']))
